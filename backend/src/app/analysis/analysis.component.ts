...
export class AnalysisComponent implements OnInit {
  ...
  public showPrintOnly: boolean = false;
    ...
    this.remoteService.getAnalysisData().subscribe((data: any) => {
              ...
              if (this.data[si]) {
                if (this.data[si][gi]) {
                  if (this.data[si][gi][bi]) {
                    this.data[si][gi][bi] = this.data[si][gi][bi] + parseInt(book.number);
                  } else {
      ...
      this.grades.forEach((grade) => {
        ...
        this.orders.filter((order) => order.user.grade.startsWith(grade)).forEach((order) => {
          order.order.forEach((book: any) => {
            this.total[gi] = (this.total[gi] ? this.total[gi] + book.number : book.number);
